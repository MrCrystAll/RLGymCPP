...
int main() {
	...
	std::cout << "Steps per second (amount of times the environment got stepped): " << (stepsPerSecond / timeSpent) << std::endl;
	std::cout << "Ticks per second (amount of times the environment did a tick (1 step = 8 ticks with RepeatAction(8)): " << (stepsPerSecond * 8 / timeSpent) << std::endl;
	std::cout << "Obs built per second (amount of observations built in a second): " << (obsBuiltPerSecond / timeSpent) << std::endl;
}